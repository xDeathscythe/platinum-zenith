...
export default async function handler(req, res) {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
        <tr style="border-bottom: 1px solid #eee;">
          <td style="padding: 10px 0; font-weight: bold; color: #555; width: 140px;">Program:</td>
          <td style="padding: 10px 0; color: #111;">${program}</td>
        </tr>
        <tr style="border-bottom: 1px solid #eee;">
          <td style="padding: 10px 0; font-weight: bold; color: #555;">Ime i prezime:</td>
          <td style="padding: 10px 0; color: #111;">${name}</td>
        </tr>
        <tr style="border-bottom: 1px solid #eee;">
          <td style="padding: 10px 0; font-weight: bold; color: #555;">Telefon:</td>
          <td style="padding: 10px 0; color: #111;"><a href="tel:${phone}">${phone}</a></td>
        </tr>
        <tr style="border-bottom: 1px solid #eee;">
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"Platinum Zenith" <${process.env.SMTP_USER}>`,
      to: email,
      subject: 'Primili smo vašu prijavu - Platinum Zenith',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #111;">Hvala na prijavi, ${name.split(' ')[0]}!</h2>
          <p style="color: #333; line-height: 1.6;">Primili smo vašu prijavu za: <strong>${program}</strong></p>
          <p style="color: #333; line-height: 1.6;">Javićemo vam se u roku od 24h sa svim detaljima.</p>
          <br>
          <p style="color: #555;">Platinum Zenith tim</p>
          <p style="color: #999; font-size: 12px;">+381 66 816 8929 · aleksandar@platinumzenith.com</p>
        </div>
      `,
    })
