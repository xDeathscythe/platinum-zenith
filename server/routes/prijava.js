...
router.post('/prijava', async (req, res) => {
  ...
  // Save to DB
  const stmt = db.prepare('INSERT INTO submissions (type, name, email, phone, company, program) VALUES (?, ?, ?, ?, ?, ?)')
  const result = stmt.run('prijava', name, email, phone, company, program)
  ...
  try {
    ...
    db.prepare('INSERT INTO email_log (recipient, subject, type, submission_id) VALUES (?, ?, ?, ?)').run('alnen96@gmail.com', `Nova prijava: ${program}`, 'notification', submissionId)
    ...
    db.prepare('INSERT INTO email_log (recipient, subject, type, submission_id) VALUES (?, ?, ?, ?)').run(email, 'Primili smo vašu prijavu', 'confirmation', submissionId)
