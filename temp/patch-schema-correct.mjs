...
const start = file.lastIndexOf(startMarker);
const end = file.indexOf(endMarker, start);
if (start === -1 || end === -1 || end <= start) throw new Error('schema markers not found');
    ...
    "@graph": [
      {
        ...
        "description": "Realni rasponi cena SEO optimizacije u Srbiji kroz pakete, rokove i očekivane rezultate po fazama rasta.",
        "inLanguage": "sr-RS",
        ...
        "itemListElement": [
          {
            ...
            "item": { "@type": "Thing", "name": "Tehnički audit sajta, istraživanje ključnih reči i optimizacija postojećeg sadržaja." }
          },
          {
            "@type": "ListItem",
            "position": 2,
            "name": "Mesec 3-4: Sadržaj i linkovi",
            "item": { "@type": "Thing", "name": "Kreiranje novog sadržaja i početak link building aktivnosti na prioritetnim temama." }
          },
          {
            "@type": "ListItem",
            "position": 3,
            "name": "Mesec 5-6: Rast i rezultati",
            "item": { "@type": "Thing", "name": "Značajniji rast organskog saobraćaja i fokus na konverzije iz SEO kanala." }
          }
        ]
      },
      {
